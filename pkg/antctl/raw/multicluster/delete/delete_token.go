...
func deleteTokenRunE(cmd *cobra.Command, args []string) error {
	...
	if deleteErr := common.DeleteMemberToken(cmd, deleteTokenOpts.k8sClient, args[0], deleteTokenOpts.namespace); deleteErr != nil {
		return deleteErr
	}

	fmt.Fprintf(cmd.OutOrStdout(), "Token %s deleted successfully", args[0])
